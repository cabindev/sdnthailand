...
export async function GET(request: NextRequest, props: { params: Promise<{ id: string }> }): Promise<NextResponse<ViewResponse>> {
...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }): Promise<NextResponse<ViewResponse>> {
  const params = await props.params;
  if (!params.id) {
    return NextResponse.json(
      { success: false, error: 'Post ID is required' },
